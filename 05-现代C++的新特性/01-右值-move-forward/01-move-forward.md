...
# C++ 隐式删除移动语义和右值构造
- 默认隐式定义：
  - 只有当类没有显式声明任何拷贝 / 移动 / 析构函数，且所有成员都可移动时，移动操作才会被隐式定义。
- 隐式删除的常见场景：
  - 类显式定义了析构函数。
  - 类显式定义了任何拷贝操作。
  - 类包含不可移动的成员。
...
C++11中，每个容器都提供了 emplace方法来添加元素。相对于 insert 有什么优点?

## emplace 的效果
当使用对象做参数时，emplace和 insert没有区别。都是利用拷贝构造，这会导致额外对象的构造和析构.
但emplace支持使用构造函数的参数做参数，这时不会产生临时对象，而是直接在容器内部调用对应的普通构造函数。
```cc
vector<Test> v;
v.reserve(100);

cout << "--------------------------" << endl;
Test t1(1);         // Test::Test(int)
