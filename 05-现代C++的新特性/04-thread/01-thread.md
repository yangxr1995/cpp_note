...
- unique_lock 实现可移动的互斥体所有权包装器
  - condition_variable 只接受 unique_lock
```cc
...
线程调用wait时，首先会释放持有的锁，并阻塞在条件变量上，等待被其他线程唤醒。
当被其他线程唤醒后，线程会立即用mutex.lock获得锁，非trylock,
所以若获得锁失败，则线程会阻塞在锁上。
```cc
void
...
当线程获得锁成功后，可能执行条件已经不再成立。
因为可能有其他线程更早获得锁并消费完了资源。
所以线程需要判断资源是否存在，这步需要程序员实现。
如果资源不存在，应该再次wait，阻塞在条件变量上。
```cc
lock(); // 先获得锁
while (!p()) // 如果条件不成立
    c.wait(lock); // 释放锁，阻塞在条件变量上
                  // 因条件变量被唤醒后，首先获得锁
                  // 由于锁不一定获得，所以可能阻塞在锁上
                  // 导致执行条件可能不成立，
                  // 所以获得锁后需要再次检查条件
...
wait_for(unique_lock<mutex>& __lock,
   ...
   _Predicate __p)
```
wait_for在wait的基础上提供了超时时间，
并通过返回值判断是否是由于超时导致的返回。超时返回false

wait_until也提供了超时时间，但时间类型是时钟时间，wait_for超时时长
